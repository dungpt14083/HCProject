...
public class ScratchCard : MonoBehaviour
{
    ...
    [SerializeField] private GameObject rewardSpecial;

    [SerializeField] private TextMeshProUGUI valueTextNormal;
    [SerializeField] private Image iconReward;
    [SerializeField] private TextMeshProUGUI valueTextSpecial;


    public void ShowView(int value, int type, string nameReward)
    {
        SetDefault();
        switch (type)
        {
            case 1:
                rewardNormal.SetActive(true);
                iconReward.sprite = ResourceManager.Instance.GetIconRewardBonusGameSmall(RewardType.Gold);
                iconReward.SetNativeSize();
                valueTextNormal.text = StringUtils.FormatMoneyK(value);
                break;
            case 2:
                rewardNormal.SetActive(true);
                iconReward.sprite = ResourceManager.Instance.GetIconRewardBonusGameSmall(RewardType.Token);
                iconReward.SetNativeSize();
                valueTextNormal.text = StringUtils.FormatMoneyK(value);
                break;
            case 3:
                rewardNormal.SetActive(true);
                iconReward.sprite = ResourceManager.Instance.GetIconRewardBonusGameSmall(RewardType.Ticket);
                iconReward.SetNativeSize();
                valueTextNormal.text = StringUtils.FormatMoneyK(value);
                break;
            default:
                rewardSpecial.SetActive(true);
                valueTextSpecial.text = nameReward;
                break;
        }
    }
    ...
    {
        rewardNormal.gameObject.SetActive(false);
        rewardSpecial.gameObject.SetActive(false);
    }
    ...
    private void GetScratchBounds()
    {
        ...
        else
        {
            ...
            if (surfaceRectTransform != null)
            {
                ...
                boundsSize = MainCamera.orthographic
